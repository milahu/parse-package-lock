...
export async function parsePackageLock({ packagePath, lockfilePath, eventHandlers, requireDependencies }) {

  // eventHandlers: onPackage, eventHandlers.error, eventHandlers.info
  ...
  */
  ...
  if (requireDependencies.peer != false) { // null or true
    dependenciesSpec.push({
      object: packageMeta.peerDependencies,
      required: requireDependencies.peer == true,
    });
  }

  if (requireDependencies.dev != false) { // null or true
    dependenciesSpec.push({
      object: packageMeta.devDependencies,
      required: requireDependencies.dev == true,
    });
  }

  await lockfile_parser({ packagePath, lockfilePath, dependenciesSpec, eventHandlers });
}



export async function parsePackageLockNpm({ packagePath, lockfilePath, dependenciesSpec, eventHandlers }) {
  ...
  */
  ...
  function walk({ lockData, name, spec, required, eventHandlers, depth, parents, parentNames, rootNode }) {
    ...
    for (const [depName, edge] of lockData.edgesOut.entries()) {
      ...
      if (!dep) {
        ...
        */
        ...
        eventHandlers.error(error);
        //eventHandlers.info(`edge.to is ${edge.to} for edge ${JSON.stringify(edge, null, 2)} -> ignore this node`);
        eventHandlers.info(`ignoring optional peer-dependency ${dep.location}.`);
        continue;
      }
      ...
      if (!meta.integrity) {
        //eventHandlers.info(`meta ${JSON.stringify(meta, null, 2)}. dep ${JSON.stringify(dep, null, 2)}`);
        //eventHandlers.info(`meta ${JSON.stringify(meta, null, 2)}`);
        if (meta.inBundle) {
          // TODO what exactly means `meta.inBundle == true`?
          // = package is bundled by a parent package
          // but then, why is it in the lockfile?
          eventHandlers.info(`ignoring integrity-less bundled dependency ${dep.location}`);
          continue;
        }
        if (dep.dev) {
          eventHandlers.info(`ignoring integrity-less dev-dependency ${dep.location}`);
          continue;
        }
        eventHandlers.info(`ignoring integrity-less dependency ${dep.location}`);
        // TODO why is `dep.bundler` sometimes undefined, sometimes string?
        continue;
      }
      /*
      if (meta.inBundle) {
        eventHandlers.info(`ignoring bundled dependency ${dep.location}`);
        // TODO why is `dep.bundler` sometimes undefined, sometimes string?
        continue;
      }
      */
      ...
      if (isCycle == false && dep.edgesOut.size > 0) {
        // recurse
        walk({ lockData: dep, name: null, spec: null, required, eventHandlers, depth: depth + 1, parents: [...parents, packageData], parentNames: new Set([...parentNames, packageData.name]), rootNode });
      }
  ...
  for (const ds of dependenciesSpec) {
    ...
    for (const [name, spec] of Object.entries(ds.object)) {
      walk({ lockData, name, spec, required: ds.required, eventHandlers });
    }
  }
}
...
export async function parsePackageLockYarn2({ packagePath, lockfilePath, dependenciesSpec, eventHandlers }) {
  ...
  */
  ...
  function walk({ lockData, name, spec, required, eventHandlers, depth, parents, parentNames, versionCache }) {
    ...
    if (spec.startsWith("workspace:")) {
      eventHandlers.info(`TODO implement yarn workspace protocol. name@spec = ${name}@${spec}`);
      return;
    }
    ...
    if (isCycle == false && dep.dependencies) {
      for (const [depName, depSpec] of Object.entries(dep.dependencies)) {
        // recurse
        walk({ lockData, name: depName, spec: depSpec, required, eventHandlers, depth: depth + 1, parents: [...parents, packageData], parentNames: new Set([...parentNames, packageData.name]), versionCache });
      }
    }
    eventHandlers.leavePackage(packageData);
  }
...
export async function parsePackageLockYarn1({ packagePath, lockfilePath, dependenciesSpec, eventHandlers }) {
  ...
  function walk({ lockData, name, spec, required, eventHandlers, depth, parents, parentNames }) {
    if (!depth) depth = 1;
    if (!parents) parents = [];
    if (!parentNames) parentNames = new Set();
    ...
    */
    ...
    if (isCycle == false && dep.dependencies) {
      for (const [depName, depSpec] of Object.entries(dep.dependencies)) {
        // recurse
        walk({ lockData, name: depName, spec: depSpec, required, eventHandlers, depth: depth + 1, parents: [...parents, packageData], parentNames: new Set([...parentNames, packageData.name]) });
      }
    }
    eventHandlers.leavePackage(packageData);
  }
...
export async function parsePackageLockPnpm({ packagePath, lockfilePath, dependenciesSpec, eventHandlers }) {
  ...
  async function walk({ lockData, name, spec, required, eventHandlers, depth, parents, parentNames, versionCache }) {
    if (!depth) depth = 1;
    if (!parents) parents = [];
    if (!parentNames) parentNames = new Set();
    ...
    if (spec.startsWith("workspace:")) {
      ...
      if (spec.startsWith("/")) {
        ...
        */
        ...
        eventHandlers.info(`found alias from ${name} to ${name2} @ ${spec2}`);
        name = name2;
        spec = spec2;
      }
      ...
      if (version == null) {
        ...
        error.code = ErrorCode.NotResolved;
        if (required) eventHandlers.error(error); else eventHandlers.info(error);
        return;
      }
    ...
    if (isCycle == false && node.dependencies) {
      for (const [name, spec] of Object.entries(node.dependencies)) {
        // recurse
        await walk({ lockData, name, spec, required, eventHandlers, depth: depth + 1, parents: [...parents, packageData], parentNames: new Set([...parentNames, packageData.name]), versionCache });
      }
    }
    eventHandlers.leavePackage(packageData);
  }
  for (const ds of dependenciesSpec) {
    if (!ds.object) continue;
    for (const [name, spec] of Object.entries(ds.object)) {
      await walk({ lockData, name, spec, required: ds.required, eventHandlers });
    }
  }
}
