...
export async function parsePackageLock({ packagePath, lockfilePath, onPackage }) {
  ...
  if (!lockfilePath) {
    ...
    const lockfile_name = packageLockFilenames.find(filename => lockfile_set.has(filename));
    if (!lockfile_name) {
      throw new Error("not found lockfile in " + packagePath);
    }
    lockfilePath = path.join(packagePath, lockfile_name);
  ...
  // TODO keep separate: dependencies devDependencies peerDependencies
  var specObj = {
  ...
  return await lockfile_parser({ packagePath, lockfilePath, specObj, onPackage });
}



export async function parsePackageLockNpm({ packagePath, onPackage }) {
  ...
  var walk = function thisFunction(pkg, onPackage, depth = 0, parents = [], parentNames = new Set(), rootNode = null) {
    ...
    for (const [name, edge] of pkg.edgesOut.entries()) {
      ...
      if (isCycle == false && pkg.edgesOut.size > 0) {
        // recurse
        thisFunction(pkg, onPackage, depth+1, [...parents, pkg], new Set([...parents, pkg.name]), rootNode);
      }
    }
  }
  walk(lockData, onPackage);
}



export async function parsePackageLockYarn({ lockfilePath, specObj, onPackage }) {
  ...
  var walk = function thisFunction(lockData, name, spec, onPackage, depth = 1, parentNames = new Set(), parents = []) {
    ...
    if (isCycle == false && node.dependencies) {
      for (const [n, v] of Object.entries(node.dependencies)) {
        thisFunction(lockData, n, v, onPackage, depth+1, new Set([...parentNames, name]), [...parents, pkg]);
      }
    }
  }
  for (const [name, version] of Object.entries(specObj)) {
    walk(lockData, name, version, onPackage);
  }
}



export async function parsePackageLockPnpm({ packagePath, lockfilePath, specObj, onPackage }) {
  ...
  var lockData = await pnpm.readWantedLockfile(packagePath, {
  ...
  var walk = async function thisFunction(lockData, name, spec, onPackage, depth = 1, parentNames = new Set(), parents = [], versionCache = null) {
    if (!versionCache) {
    ...
    */
    ...
    if (isCycle == false && node.dependencies) {
      for (const [n, v] of Object.entries(node.dependencies)) {
        thisFunction(lockData, n, v, onPackage, depth+1, new Set([...parentNames, name]), [...parents, pkg], versionCache);
      }
    }
  }
  for (const [name, version] of Object.entries(specObj)) {
    await walk(lockData, name, version, onPackage);
  }
}
